...
/* Whether the ISR has a job. */
static uint8_t volatile sl_active = 0;
...
ISR(SPI_STC_vect) {
	...
	if (rx_buffers[tmp].amt) {
		 ...
		 * transaction the slave can at any point send
	...
	if (act) {
		...
		/* That was it, no more data... indicate to main. */
		/* Leave SS out 0 for less idle power waste. */
		timer_set_waiting();
		sl_active = 0;
	}
}
...
void slmaster_run(void) {
	...
	if (sl_active) return; /* If the ISR is doing stuff, let it just do stuff.. */
	if ( (tx_buffers[tx_bufwp].amt)||(timer_get_5hzp())||(trigger) ) {
		/* Initiate a transaction. */
		PORTD |= _BV(3);
		uint8_t amt = tx_buffers[tx_bufwp].amt;
		rx_buffers[rx_bufwp].off = 0;
